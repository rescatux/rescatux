...
$(call qemu_base,x86_64) \
	...
	-drive if=pflash,format=raw,file=$(OVMF_VARS_FD)
endef
...
$(call qemu_base,i386) \
	...
	-drive if=pflash,format=raw,file=$(OVMF32_VARS_FD)
endef
...
run-amd64-uefi-sb-iso:
	...
	$(call qemu_uefi_sb_amd64) -drive file=$(RUN_ISO_amd64),format=raw,if=virtio,readonly=on,media=cdrom

run-amd64-uefi-sb-usb:
	$(call check_ovmf,$(OVMF_CODE_SECB_FD),OVMF_CODE.secboot.fd)
	$(call check_ovmf,$(OVMF_VARS_FD),OVMF_VARS.fd)
	$(call qemu_uefi_sb_amd64) -drive file=$(RUN_IMG_amd64),format=raw,if=virtio,readonly=on

# AMD64 UEFI (no SecureBoot)
run-amd64-uefi-iso:
	$(call check_ovmf,$(OVMF_CODE_FD),OVMF_CODE.fd)
	$(call check_ovmf,$(OVMF_VARS_FD),OVMF_VARS.fd)
	$(call qemu_uefi_amd64) -drive file=$(RUN_ISO_amd64),format=raw,if=virtio,readonly=on,media=cdrom

run-amd64-uefi-usb:
	$(call check_ovmf,$(OVMF_CODE_FD),OVMF_CODE.fd)
	$(call check_ovmf,$(OVMF_VARS_FD),OVMF_VARS.fd)
	$(call qemu_uefi_amd64) -drive file=$(RUN_IMG_amd64),format=raw,if=virtio,readonly=on
...
run-i386-uefi-sb-iso:
	...
	$(call qemu_uefi_sb_i386) -drive file=$(RUN_ISO_i386),format=raw,if=virtio,readonly=on,media=cdrom

run-i386-uefi-sb-usb:
	$(call check_ovmf,$(OVMF32_CODE_SECB_FD),OVMF32_CODE_4M.secboot.fd)
	$(call check_ovmf,$(OVMF32_VARS_FD),OVMF32_VARS.fd)
	$(call qemu_uefi_sb_i386) -drive file=$(RUN_IMG_i386),format=raw,if=virtio,readonly=on

# I386 UEFI (no SecureBoot)
run-i386-uefi-iso:
	$(call check_ovmf,$(OVMF32_CODE_FD),OVMF32_CODE_4M.fd)
	$(call check_ovmf,$(OVMF32_VARS_FD),OVMF32_VARS.fd)
	$(call qemu_uefi_i386) -drive file=$(RUN_ISO_i386),format=raw,if=virtio,readonly=on,media=cdrom

run-i386-uefi-usb:
	$(call check_ovmf,$(OVMF32_CODE_FD),OVMF32_CODE_4M.fd)
	$(call check_ovmf,$(OVMF32_VARS_FD),OVMF32_VARS.fd)
	$(call qemu_uefi_i386) -drive file=$(RUN_IMG_i386),format=raw,if=virtio,readonly=on
